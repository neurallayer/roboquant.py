...
# %%
# Configuration for the strategy and feed
spy = Stock("SPY")
history_period = 20  # use 20 trading days of history for the input features
prediction_period = 5  # predict 5 trading days in the future
...
label = MaxReturnFeature(PriceFeature(spy, price_type="HIGH"), prediction_period)

model = TimeSeriesTransformer(feature_size=input.size(), seq_length=history_period, label_size=label.size())

# Create the strategy
logging.basicConfig()
logger.setLevel("INFO")
strategy = TimeSeriesStrategy(input, label, model, spy, sequences=history_period, buy_pct=0.02, sell_pct=0.02)

# %%
# Train the model from 2000 to 2020
tf = rq.Timeframe.fromisoformat(start_date, "2020-01-01")
strategy.fit(feed, timeframe=tf, epochs=20, validation_split=0.25, prediction=prediction_period, warmup=100)
