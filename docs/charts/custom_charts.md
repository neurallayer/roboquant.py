...
This example shows how to use some `IndicatorMetric`s and the `SignalRatingMetric` to track indicators and signals on a chart.
...
Let's plot the price and Bollinger Bands on the first axis. We use the `track` method of the feed to track the Bollinger Bands metric, which returns a TimeSeries DataFrame with the lower and upper bands.
