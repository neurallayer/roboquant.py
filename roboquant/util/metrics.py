...
class SignalRatingMetric(Metric):
    """Tracks the signal ratings generated by a strategy for one or more assets. It will
    track the signal rating for each asset or NaN if no signal was generated for the asset in the current event.
    The metric name is `rating/<asset_symbol>` and the value is the signal rating.
    """

    def __init__(self, *assets: Asset, strategy: Strategy):
        self.assets = assets
    ...
    def calc(self, event: Event, account: Account, signals: list[Signal], orders: list[Order]) -> dict[str, float]:
        ...
        for asset in self.assets:
            ...
            if signal := d.get(asset):
                result[metric_name] = signal.rating
            else:
